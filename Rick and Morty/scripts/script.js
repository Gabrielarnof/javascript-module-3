...
async function fetchLocationCharacters(charactersUrls) {
  ...
  const unresolvedFetchPromises = charactersUrls.map((characterUrl) =>
    fetch(characterUrl)
  );
  const resolvedFetchPromises = await Promise.all(unresolvedFetchPromises);
  const jsonPromises = resolvedFetchPromises.map((resolvedPromise) =>
    resolvedPromise.json()
  );
...
async function fetchCharacterEpisodes(episodesUrls) {
  //map url to fetch promises
  const unresolvedFetchPromises = episodesUrls.map((characterUrl) =>
    fetch(characterUrl)
  );
  const resolvedFetchPromises = await Promise.all(unresolvedFetchPromises);
  const jsonPromises = resolvedFetchPromises.map((resolvedPromise) =>
